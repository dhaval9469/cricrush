...
Widget showFlag({required BuildContext context, required String url, double borderRadius = 50, double w = 0, double h = 0}) {
  ...
  return ClipRRect(
    ...
    child: url.isEmpty
        ? Image.asset(AppAssets.dFlag, height: height, width: width, fit: BoxFit.cover)
        : CachedNetworkImage(
            imageUrl: url,
            height: height,
            width: width,
            ...
            errorWidget: (context, url, error) => Image.asset(AppAssets.dFlag, height: height, width: width, fit: BoxFit.cover),
          ),
  );
}

Widget showPlayer({required BuildContext context, required String url, double w = 0, double h = 0, double r = 0}) {
  double height = h == 0 ? context.wp(5.5) : context.wp(h);
  double width = w == 0 ? context.wp(5.5) : context.wp(w);
  ...
  return ClipRRect(
    ...
    child: url.isEmpty
        ? Image.asset(AppAssets.dummyPlayer, height: height, width: width, fit: BoxFit.cover)
        : CachedNetworkImage(
            imageUrl: url,
            height: height,
            width: width,
            ...
            errorWidget: (context, url, error) => Image.asset(AppAssets.dummyPlayer, height: height, width: width, fit: BoxFit.cover),
          ),
  );
}
